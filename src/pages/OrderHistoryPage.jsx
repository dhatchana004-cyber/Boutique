      ...
      {steps.map((step, i) => (
        <div key={step} className="flex items-center flex-1 last:flex-none">
          <div className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 text-white
            ...
            ${i <= activeIdx ? 'bg-[#D4AF37]' : 'bg-[#1A1A1A]'}`}>
            {i < activeIdx ? (
...
function OrderCard({ order, onCancel }) {
  ...
  return (
    <div className="border border-[#333333] rounded-2xl overflow-hidden
      hover:border-[#444444] hover:shadow-sm transition-all duration-200">
      ...
      <div className="p-5 flex items-start justify-between">
        <div>
          ...
          <p className="text-sm font-semibold text-gray-100">Placed {date}</p>
          ...
          <div className="flex justify-between mt-1">
            {['Confirmed', 'Shipped', 'Delivered'].map((s, i) => (
              <p key={i} className={`text-[10px] font-semibold
                ${i === (order.status === 'PROCESSING' ? 0 : 1) ? 'text-[#D4AF37]' : 'text-gray-400'}`}>
                {s}
              </p>
            ))}
          </div>
      ...
      <div className="px-5 pb-4">
        <div className="flex flex-col gap-3">
          {(open ? order.items : order.items.slice(0, 2)).map(item => (
            ...
            >
              <div className="w-14 h-14 rounded-xl overflow-hidden bg-[#111111] flex-shrink-0
                ring-1 ring-gray-100 group-hover:ring-[#D4AF37] transition-all">
                <img
          ...
          {order.items.length > 2 && (
            <button
              onClick={() => setOpen(o => !o)}
              className="text-xs text-[#D4AF37] font-bold hover:underline self-start"
            >
              {open
                ? '↑ Show less'
          ...
          {order.status === 'DELIVERED' && (
            <button
              ...
              className="px-4 py-2 bg-gray-900 text-white text-xs font-bold
                tracking-wider uppercase rounded-full hover:bg-[#D4AF37] transition-all"
            >
              Buy Again
            </button>
          )}
          {order.status === 'SHIPPED' && (
            <button className="px-4 py-2 bg-[#D4AF37] text-white text-xs font-bold
              tracking-wider uppercase rounded-full hover:bg-[#FFFFFF] hover:text-[#000000] transition-all">
              Track Package
            </button>
          )}
        </div>
...
function OrderSkeleton() {
  return (
    <div className="border border-[#333333] rounded-2xl p-5 animate-pulse">
      ...
      {[1, 2].map(i => (
        <div key={i} className="flex gap-3 mb-3">
          <div className="w-14 h-14 bg-[#1A1A1A] rounded-xl flex-shrink-0" />
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-6 bg-[#000000]">
        <div className="w-20 h-20 bg-[#111111] rounded-full flex items-center justify-center">
          <svg className="w-10 h-10 text-[#D4AF37]" fill="none" viewBox="0 0 24 24"
        ...
        <h1 className="text-2xl font-black text-gray-100">Please Sign In</h1>
        ...
        <div className="flex gap-10">
          ...
          <aside className="hidden md:block w-56 flex-shrink-0">
            <div className="flex items-center gap-3 mb-8">
              <img
                ...
                alt={user.name}
                className="w-12 h-12 rounded-full ring-2 ring-[#333333]"
              />
              <div>
                <p className="font-bold text-gray-100 text-sm">{user.name}</p>
            ...
            <div className="mt-8 p-4 bg-[#111111] rounded-2xl">
              <p className="text-sm font-bold text-gray-100 mb-1">Need Assistance?</p>
              <p className="text-xs text-gray-400 mb-3">
                Our digital concierge is available 24/7 for tailored support.
              </p>
              <button className="text-xs text-[#D4AF37] font-bold hover:underline">
                Contact Specialist →
              </button>
            </div>
          </aside>
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="mb-8 flex items-start justify-between">
              <div>
                <h1 className="text-3xl font-black text-gray-100 mb-2">Order History</h1>
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 text-xs font-bold text-gray-500
                  border border-[#444444] rounded-full hover:border-gray-400 transition-all
                  disabled:opacity-50"
              >
              ...
              {TABS.map(tab => (
                ...
                >
                  ...
                  <span className={`text-[10px] px-1.5 py-0.5 rounded-full
                    ${activeTab === tab ? 'bg-[#000000]/20' : 'bg-[#222222]'}`}>
                    {countFor(tab)}
                  </span>
                </button>
              ))}
            ...
            {error && (
              <div className="flex flex-col items-center justify-center py-16 text-center">
                ...
                <p className="text-gray-100 font-bold mb-2">Something went wrong</p>
            ...
            {!loading && !error && filtered.length === 0 && (
              <div className="flex flex-col items-center justify-center py-20 bg-[#111111] rounded-3xl border border-[#333333]">
                <p className="text-5xl mb-4">📦</p>
                <p className="text-gray-100 font-bold text-lg">No orders found</p>
                ...
                <button
                  onClick={() => navigate('/products')}
                  className="px-8 py-3 bg-[#D4AF37] text-white text-xs font-bold tracking-widest uppercase rounded-full hover:shadow-lg transition-all"
                >
                  Start Shopping
                </button>
              </div>
