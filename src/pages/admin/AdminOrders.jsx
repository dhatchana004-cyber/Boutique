    ...
    <>
      <div className="bg-[#000000] rounded-2xl shadow-sm border border-[#333333] overflow-hidden">
        <div className="px-6 py-5 border-b border-[#333333] flex items-center justify-between">
          <h3 className="font-bold text-gray-100">All Orders</h3>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-50">
              {orders.map(o=>(
                <tr key={o.id} className="hover:bg-[#111111]/50">
                  <td className="px-6 py-4 font-mono text-xs font-bold">#{o.id}</td>
                  <td className="px-6 py-4">
                    <p className="font-semibold text-gray-100 text-sm">{o.user?.name||'—'}</p>
                    <p className="text-xs text-gray-400">{o.user?.email}</p>
                  </td>
                  <td className="px-6 py-4 font-bold">₹{o.total?.toLocaleString()}</td>
                  <td className="px-6 py-4">
                    <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase ${STATUS_COLORS[o.paymentStatus]||'bg-[#1A1A1A] text-gray-500'}`}>{o.paymentStatus}</span>
                  </td>
                  <td className="px-6 py-4">
                    {editing===o.id ? (
                      <select defaultValue={o.status} onChange={e=>handleStatus(o.id,e.target.value)} onBlur={()=>setEditing(null)} autoFocus
                        className="text-xs border border-blue-300 rounded-lg px-2 py-1 outline-none bg-[#000000]">
                        {STATUSES.map(s=><option key={s} value={s}>{s}</option>)}
                      </select>
                    ) : (
                      <button onClick={()=>setEditing(o.id)} className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase cursor-pointer hover:ring-2 ring-blue-300 transition-all ${STATUS_COLORS[o.status]||'bg-[#1A1A1A] text-gray-500'}`}>
                        {o.status}
                      </button>
                    )}
                  </td>
      ...
      {selected && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={()=>setSelected(null)}>
          <div className="bg-[#000000] rounded-2xl shadow-2xl w-full max-w-lg mx-4 max-h-[80vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 py-4 border-b border-[#333333]">
              <h3 className="font-bold text-gray-100">Order #{selected.id}</h3>
              <button onClick={()=>setSelected(null)} className="p-1 hover:bg-[#1A1A1A] rounded-lg"><X size={18}/></button>
            </div>
            <div className="p-6 space-y-4">
              ...
              <div className="border-t pt-3 flex justify-between">
                ...
                <span className="font-black text-lg text-[#D4AF37]">₹{selected.total?.toLocaleString()}</span>
              </div>
            </div>
          </div>
        </div>
